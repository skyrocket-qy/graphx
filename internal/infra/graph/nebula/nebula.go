...
func main() {
	...
	if err != nil {
		log.Fatal(fmt.Sprintf(
			"Fail to create a new session from connection pool,"+
	...
	{
		// Prepare the query
		createSchema := "CREATE SPACE IF NOT EXISTS basic_example_space" +
		...
		insertSchema := "INSERT EDGE e1 () VALUES \"10\"->\"11\":();"
		resultSet, err = session.Execute(insertSchema)
		if err != nil {
			fmt.Print(err.Error())
			return
		}
		checkResultSet(insertSchema, resultSet)
	}
	// Drop space
	{
